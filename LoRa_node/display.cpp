...
SoftwareSerial Display_serial(DPS_RX_PIN, DPS_TX_PIN);
...
void Display::send_data_to_display(const char *data)
{
  Serial.println(data);
...
int Display::get_data_from_display(char *buffer)
{
...
void Display::print_page_on_display(int page_num)
{
  if (page_num == -1)
  {
    this->send_data_to_display("page loading_page");
