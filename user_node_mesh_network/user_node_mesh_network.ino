...
void processReceivedPackets(void*) {
    for (;;) {
        ...
        while (radio.getReceivedQueueSize() > 0) {
            ...
            dataPacket* dPacket = packet->payload;
            String string_data_str = String(dPacket[0].data_str);
            Serial.println(string_data_str);

            if(string_data_str.startsWith("SEND_CMD") > 0)
            {
              Serial.println("Jump here?");
              int pos = strlen("SEND_CMD");
              // char temp_str[30];
              // memset(temp_str, 0, 30);
              // strncpy(temp_str, +(pos+1),strlen());
              Serial.println(string_data_str.substring(pos+1));
              Serial2.println(string_data_str.substring(pos+1));
            }
