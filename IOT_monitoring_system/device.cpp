...
IOT_device::IOT_device()
{
  ...
  this->_store_db_msg_queue = xQueueCreate(2, sizeof(int));
...
int IOT_device::save_data_to_database()
{
  ...
  sprintf(this->_sensor_buffer,"%d,%d,%d,%.2f,%.2f,%d,%s\n",
    ...
    // store to database
  while (this->_database.append_file(SD, SENSOR_RECORD_FILE_PATH, this->_sensor_buffer, true) != DATABASE_SUCCESS_WRITE_FILE)
  {
    i++;
...
void IOT_device::handle_save_to_database()
{
  ...
  if (xQueueReceive(this->_store_db_msg_queue, (void *)&item, tick) == pdTRUE)
  {
    if (item == SAVE_DATA_SENSOR)
    {
...
void IOT_device::calculate_sensor_data()
{
  ...
  this->_sensors_data_bucket.n2o_data      = this->_sensors_data_bucket.n2o_data      / no_of_sample;
...
void IOT_device::handle_save_setting()
{
  ...
  // char string_buffer[5] = {0,0,0,0,0};
  ...
  for (int i = 0; i < len; i++)
  {
    ...
    // strncat(this->_setting_buffer, "EVENT_TIME_SET,", strlen("EVENT_TIME_SET,"));

    // memset(string_buffer, 0, 5);
    // sprintf(string_buffer, "%d", this->_measure_events.event_time.time[i].hours);
    // string_buffer[strlen(string_buffer)] = ',';
    // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));

    // memset(string_buffer, 0, 5);
    // sprintf(string_buffer, "%d", this->_measure_events.event_time.time[i].minutes);
    // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));

    // strncat(this->_setting_buffer, &c, 1);
  ...
  // strncat(this->_setting_buffer, "MAJOR_TIME_SET,", strlen("MAJOR_TIME_SET,"));

  // memset(string_buffer, 0, 5);
  // sprintf(string_buffer, "%d", this->_measure_events.major_time.number_of_intervals);
  // string_buffer[strlen(string_buffer)] = ',';
  // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));

  // memset(string_buffer, 0, 5);
  // sprintf(string_buffer, "%d", this->_measure_events.major_time.time);
  // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));
  ...
  // strncat(this->_setting_buffer, "MINOR_TIME_SET,", strlen("MINOR_TIME_SET,"));

  // memset(string_buffer, 0, 5);
  // sprintf(string_buffer, "%d", this->_measure_events.minor_time.number_of_intervals);
  // string_buffer[strlen(string_buffer)] = ',';
  // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));

  // memset(string_buffer, 0, 5);
  // sprintf(string_buffer, "%d", this->_measure_events.minor_time.time);
  // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));
  ...
  // strncat(this->_setting_buffer, "SPIKE_TIME_SET,", strlen("SPIKE_TIME_SET,"));

  // memset(string_buffer, 0, 5);
  // sprintf(string_buffer, "%d", this->_measure_events.spike_time.number_of_intervals);
  // string_buffer[strlen(string_buffer)] = ',';
  // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));

  // memset(string_buffer, 0, 5);
  // sprintf(string_buffer, "%d", this->_measure_events.spike_time.time);
  // strncat(this->_setting_buffer, string_buffer, strlen(string_buffer));

  // strncat(this->_setting_buffer, &c, 1);
    Serial.println(this->_setting_buffer);
  this->_database.append_file(SD, SETTING_FILE_PATH, this->_setting_buffer,false);
  // this->_setting_buffer
}

// void IOT_device::handle_load_setting_from_database()
// {
//   int line_total = 0;
//   char cmd_buffer[50];

//   if (this->_database.read_file(SD, SETTING_FILE_PATH, this->_setting_buffer, &line_total) == DATABASE_SUCCESS_READ_FILE)
//   {
//     for (int i = 0; i < line_total; i++)
//     {
//       memset(cmd_buffer, 0, 50);
//       split_string_char(this->_setting_buffer, '\n', i, cmd_buffer);
//       strcpy(this->_buffer, this->_setting_buffer);
//       handle_command_serial();
//     }
//   }
// }
...
int IOT_device::on()
{
  ...
  switch (this->_device_on_state)
  {
    ...
    case DEVICE_ON_SPIKE_TIME:
    {
      ...
      this->_actuators.set_actuators_pin(PUMP_1_PIN, HIGH);
      ...
      {
        int num = SAVE_DATA_SENSOR;
        this->calculate_sensor_data();

        if (xQueueSend(this->_store_db_msg_queue, (void *)&num, 100) != pdTRUE)
        {
    ...
    default:
    {
      _device_on_state = DEVICE_ON_IDLE;
    }
  }
...
void IOT_device::display_print_date_time()
{
  ...
  // strcat(Event_time_string_temp, "t14.txt=\"");

  // memset(temp, 0, 3);
  // convert_int_to_str(this->_rtc.get_hours(),temp);
  // strcat(Event_time_string_temp, temp);
  // strcat(Event_time_string_temp, ":");
  
  // memset(temp, 0, 3);
  // convert_int_to_str(this->_rtc.get_minutes(),temp);
  // strcat(Event_time_string_temp, temp);
  // strcat(Event_time_string_temp, "\"");
...
int IOT_device::process_cmd()
{
  ...
  else if (str_startswith(this->_buffer, "CYLINDER1_ON"))          { this->_actuators.set_actuators_pin(FAN_2_PIN, HIGH); }
  else if (str_startswith(this->_buffer, "CYLINDER1_OFF"))         { this->_actuators.set_actuators_pin(FAN_2_PIN, LOW); }
  ...
  else if (str_startswith(this->_buffer, "SEND_DTSS")) // SEND_DTSS,(data)
  {
